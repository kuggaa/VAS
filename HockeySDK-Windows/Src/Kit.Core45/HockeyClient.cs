namespace Microsoft.HockeyApp
{
	...
	public class HockeyClient : HockeyApp.IHockeyClient, IHockeyClientInternal, IHockeyClientConfigurable
	{
		...
		public async Task<bool> SendCrashesAndDeleteAfterwardsAsync ()
		{
			...
			if (Monitor.TryEnter (this)) {
				try {
					...
					try {
						Monitor.Exit (this);
					} catch (Exception ex) { //ignore. on next start it will try again.
						HandleInternalUnhandledException (ex);
					}
				}
			}
		...
		public async Task<IEnumerable<IAppVersion>> GetAppVersionsAsync ()
		{
			...
			if (!String.IsNullOrEmpty (this.Auid)) { url.Append ("&=auid" + Uri.EscapeDataString (this.Auid)); } else if (!String.IsNullOrEmpty (this.Iuid)) { url.Append ("&=iuid" + Uri.EscapeDataString (this.Iuid)); } else if (!String.IsNullOrEmpty (this.Uuid)) { url.Append ("&=duid" + Uri.EscapeDataString (this.Uuid)); }
		...
		public CrashLogInformation PrefilledCrashLogInfo {
			get {
				if (_crashLogInfo == null || !_crashLogInfo.HasValue) {
		...
		{
			Console.WriteLine ("Tracking hockeyclient");
			this.TrackEvent (new EventTelemetry (eventName));
			Console.WriteLine ("Tracked hockeyclient");
		}
		...
		internal void Track (ITelemetry telemetry)
		{
			...
			if (this.IsTelemetryEnabled ()) {
				...
				Console.WriteLine ("Track hockeyclient");
			}
		}
