...
namespace VAS.KPI.Services
{
	internal class StorageService : BaseStorageService
	{
		...
		{
			var folderOption1 = new { RootPath = Environment.GetFolderPath (Environment.SpecialFolder.LocalApplicationData), AISubFolder = @"HockeyApp" };

			if (!string.IsNullOrEmpty (folderOption1.RootPath)) {
				var root = new DirectoryInfo (folderOption1.RootPath);
				string subdirectoryPath = Path.Combine (folderOption1.AISubFolder, this.storageFolderName);
