...
public class ImplementsInterfaceFinder
{
	...
	public TypeDefinition HierarchyImplementsIStorable (TypeReference typeReference)
	{
		...
		if (!typeReferencesImplementingInterface.TryGetValue (fullName, out baseType)) {
			...
			if (ImplementsInterface (type)) {
				...
				baseType = HierarchyImplementsIStorable (type.BaseType);
			}
		}
	...
	{
		return typeDefinition.Interfaces.Any (x => x.InterfaceType.Name == "IStorable");
	}
